...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (isEmpty(headers)) {
            throw new ClientException("请求头中没有鉴权字段", BaseErrorCode.USER_REGISTER_ERROR);
        }
        token = headers.get(0);
        ...
        String key_user = GuestCacheKey.LOGIN_USER_KEY + token;
        String key_admin = GuestCacheKey.LOGIN_USER_KEY_ADMIN + token;
        ...
        String saved_info = JSONUtil.toJsonStr(new UserDTO(userId, SystemConstant.TEMP_ACCOUNT_NAME));
