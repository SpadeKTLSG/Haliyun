...
package xyz.spc.gate.gateway.config.redis.compo;
...
import xyz.spc.gate.gateway.config.redis.core.*;
import xyz.spc.serve.auxiliary.config.design.single.Singleton;
...
public class StringRedisTemplateProxy implements DistributedCache {
     ...
     * 安全获取分布式锁 key 前缀
     */
    private static final String SAFE_GET_DISTRIBUTED_LOCK_KEY_PREFIX = "sysCache:safe_get_distributed_lock_get:";
    ...
    public <T> T get(String key, Class<T> clazz) {
        ...
        return JSON.parseObject(value, JsonUtil.buildType(clazz));
    }
    ...
    public <T> T get(@NotBlank String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit) {
        ...
        if (!RedisCacheGeneral.isNullOrBlank(result)) {
            return result;
        }
    ...
    public <T> T safeGet(String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit,
        ...
        try {
            ...
            if (RedisCacheGeneral.isNullOrBlank(result = get(key, clazz))) {
                // 如果访问 cacheLoader 加载数据为空，执行后置函数操作
                if (RedisCacheGeneral.isNullOrBlank(result = loadAndSet(key, cacheLoader, timeout, timeUnit, true, bloomFilter))) {
